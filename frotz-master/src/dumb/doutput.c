...
void serialize_payload(char *buffer, struct Payload *payload) {
    memcpy(buffer, &payload->id, sizeof(int));
    memcpy(buffer + sizeof(int), &payload->counter, sizeof(int));
    memcpy(buffer + 2 * sizeof(int), payload->msg, BUFFER_SIZE);
    memcpy(buffer + 2 * sizeof(int) + BUFFER_SIZE, &payload->temp, sizeof(float));
}
...
void sendMsgToServer(int sock, void* msg, int msgsize)
{
...
void openSockClient(char *input)
{
    int sock;
    struct sockaddr_in server_address;
    struct Payload data;
    char buffer[2 * sizeof(int) + BUFFER_SIZE + sizeof(float)];
	...
	data.id = 1;
	data.counter = 0;
	strncpy(data.msg, input, BUFFER_SIZE - 1);
	data.msg[BUFFER_SIZE - 1] = '\0';
	data.temp = mintemp + (rand() / (float)RAND_MAX) * (maxtemp - mintemp);

	//printf("\nSending id=%d, counter=%d, message=%s, temp=%f\n", data.id, data.counter, data.msg, data.temp);
        
        serialize_payload(buffer, &data);
        
        sendMsgToServer(sock, buffer, BUFFSIZE);
        
        // Recebe dados do servidor
	char recv_buffer[BUFFSIZE];
	int nread = read(sock, recv_buffer, BUFFSIZE);
	if (nread < 0) {
		perror("recv error");
	} 
	
	else {
		struct Payload recv_data;
		memcpy(&recv_data, recv_buffer, BUFFSIZE);
		//printf("Received id=%d, counter=%d, message=%s, temp=%f\n", recv_data.id, recv_data.counter, recv_data.msg, recv_data.temp);
		
		fprintf(stderr, "DEBUG: recv msg (as string): '%s'\n", recv_data.msg);
		
		strncpy(input, recv_data.msg, BUFFER_SIZE - 1);
